...
func (n *V2ClientSession) GetSessionData() *SessionData {
	return &n.SessionData
}
...
func (n *V2Session) computeExpectedResponses(timestamp []byte, avPairBytes []byte) (err error) {
	...
	ntProofStr := hmac_MD5(n.responseKeyNT, concat(n.serverChallenge, temp))
	n.ntChallengeResponse = concat(ntProofStr, temp)
	n.lmChallengeResponse = concat(hmac_MD5(n.responseKeyLM, concat(n.serverChallenge, n.clientChallenge)), n.clientChallenge)
	n.sessionBaseKey = hmac_MD5(n.responseKeyNT, ntProofStr)
	return
}
...
func (n *V2ServerSession) GssEncrypt(s []byte) []byte {
	return n.ntlmV2Security.GssEncrypt(s)
}

func (n *V2ServerSession) GssDecrypt(s []byte) []byte {
	return n.ntlmV2Security.GssDecrypt(s)
}
...
func (n *V2ServerSession) ProcessNegotiateMessage(nm *NegotiateMessage) (err error) {
	...
	// TODO: process received NTLMSSP_NEGOTIATE_MESSAGE
	return
}
...
func (n *V2ServerSession) GenerateChallengeMessage() (cm *ChallengeMessage, err error) {
	...
	desktopName := "DESKTOP-PROXYSERER"
	cm.TargetName, _ = CreateStringPayload(desktopName)
	...
	flags = NTLMSSP_NEGOTIATE_56.Set(flags)       // new
	flags = NTLMSSP_TARGET_TYPE_SERVER.Set(flags) // new
	flags = NTLMSSP_NEGOTIATE_SEAL.Set(flags)     // new
	...
	pairs := new(AvPairs)
	pairs.AddAvPair(MsvAvNbDomainName, Utf16FromString(desktopName))
	pairs.AddAvPair(MsvAvNbComputerName, Utf16FromString(desktopName))
	pairs.AddAvPair(MsvAvDnsDomainName, Utf16FromString(desktopName))
	pairs.AddAvPair(MsvAvDnsComputerName, Utf16FromString(desktopName))
	pairs.AddAvPair(MsvAvTimestamp, timeToWindowsFileTime(time.Now()))
	pairs.AddAvPair(MsvAvEOL, make([]byte, 0))
	cm.TargetInfo = pairs
	cm.TargetInfoPayloadStruct, _ = CreateBytePayload(pairs.Bytes())

	cm.Version = &VersionStruct{ProductMajorVersion: uint8(10), ProductMinorVersion: uint8(0), ProductBuild: uint16(22621), NTLMRevisionCurrent: uint8(15)}
	return cm, nil
}
...
func (n *V2ServerSession) ProcessAuthenticateMessage(am *AuthenticateMessage) (err error) {
	...
	n.ntlmV2Security = NTLMv2Security{EncryptRC4: n.serverHandle, DecryptRC4: n.clientHandle, SigningKey: n.ServerSigningKey, VerifyKey: n.ClientSigningKey}
...
func (n *V2ServerSession) computeExportedSessionKey() (err error) {
	if NTLMSSP_NEGOTIATE_KEY_EXCH.IsSet(n.NegotiateFlags) {
		...
		// n.calculatedMic = HmacMd5(n.exportedSessionKey, concat(n.challengeMessage.Payload, n.authenticateMessage.Serialize))
	} else {
		n.exportedSessionKey = n.keyExchangeKey
		// TODO: Calculate mic correctly. This calculation is not producing the right results now
		// n.calculatedMic = HmacMd5(n.keyExchangeKey, concat(n.challengeMessage.Payload, n.authenticateMessage.Serialize))
	}
	return nil
}
...
func (n *V2ClientSession) GetEncodedCredentials() ([]byte, []byte, []byte) {
	if NTLMSSP_NEGOTIATE_UNICODE.IsSet(n.authenticateMessage.NegotiateFlags) {
		return Utf16FromString(n.userDomain), Utf16FromString(n.user), Utf16FromString(n.password)
	}
	return []byte(n.userDomain), []byte(n.user), []byte(n.password)
}

func (n *V2ClientSession) GssEncrypt(s []byte) []byte {
	return n.ntlmV2Security.GssEncrypt(s)
}

func (n *V2ClientSession) GssDecrypt(s []byte) []byte {
	return n.ntlmV2Security.GssDecrypt(s)
}
...
func (n *V2ClientSession) GenerateNegotiateMessage() (nm *NegotiateMessage, err error) {
	...
	nm.Version = &VersionStruct{ProductMajorVersion: uint8(10), ProductMinorVersion: uint8(0), ProductBuild: uint16(22621), NTLMRevisionCurrent: uint8(15)}
...
func (n *V2ClientSession) ProcessChallengeMessage(cm *ChallengeMessage) (err error) {
	...
	n.ntlmV2Security = NTLMv2Security{EncryptRC4: n.clientHandle, DecryptRC4: n.serverHandle, SigningKey: n.ClientSigningKey, VerifyKey: n.ServerSigningKey}
...
func (n *V2ClientSession) GenerateAuthenticateMessage() (am *AuthenticateMessage, err error) {
	...
	am.Version = &VersionStruct{ProductMajorVersion: uint8(10), ProductMinorVersion: uint8(0), ProductBuild: uint16(19041), NTLMRevisionCurrent: 0x0F}
	...
	am.Mic = hmac_MD5(n.exportedSessionKey, concat(n.negotiateMessage.Serialize(), n.challengeMessage.Serialize(), am.Serialize()))
...
func ntowfv2(user string, passwd string, userDom string) []byte {
	...
	return hmac_MD5(md4(Utf16FromString(passwd)), concat)
}
